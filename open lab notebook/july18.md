# week 2.7.-6.7.
- tried to run CPAC(https://github.com/BIDS-Apps/CPAC): 
- ip=$(ifconfig en0 | grep inet | awk '$1=="inet" {print $2}') --> get your IP adress
- xhost + ${ip} --> enable xhost to accept connections to localhost
- run CPAC: docker run -i --rm \
        --privileged \
        -e DISPLAY=$DISPLAY \
...
# problems: 
- the file structure of the bids dataset need to be folder/folder/sub/anat/bids to be recognized by CPAC
- using the GUI, I couldn't enable qc
- pipeline configuration error if Ants and FSL are anabled for the anatomical to template registration, ANTS alone works
...
crashLogDirectory :  /outputs

- Directory where CPAC should place run logs.
logDirectory :  /outputs
...
- Select which nuisance signal corrections to apply:# compcor = CompCor
- wm = White Matter
- csf = CSF
- gm = Gray Matter
...
- Directory where CPAC should store temporary and intermediate files.
workingDirectory :  /outputs/working_directory/

- Directory where CPAC should write crash logs.
crashLogDirectory :  /outputs/crash_directory/

- Directory where CPAC should place run logs.
logDirectory :  /outputs/log_directory/

- Directory where CPAC should place processed data.
outputDirectory :  /outputs/output_directory/
